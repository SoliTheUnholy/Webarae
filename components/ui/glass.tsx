 ...
 * scientific notation (from rounded-xl), percentages, and invalid values.
...
const getBorderRadius = (element: HTMLElement, rect: DOMRect): number => {
    ...
    // Handle scientific notation (e.g., '1.67772e+07px' from rounded-xl) or very large values
    if (
        parsedRadius > 9999 ||
        rawRadius.includes("e+") ||
        rawRadius.includes("E+")
    ) {
        // For very large values (like rounded-xl), return half of smallest dimension
...
>(({ children, filterId, onReady, className, ...props }, ref) => {
    ...
    const supportsSVGFilters = useCallback(() => {
        ...
        if (isWebkit || isFirefox || width < 500) {
            return false;
        }
