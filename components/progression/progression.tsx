...
function useProgression() {
    ...
    if (!context) {
        throw new Error(
            "Progression must be used within a ProgressionProvider",
        );
    }
    return context;
}
...
function ProgressionRoot({ children }: { children: React.ReactNode }) {
    ...
    const [scrollToProgressFn, setScrollToProgressFn] = useState<
    ...
    const scrollToProgress = React.useCallback(
        (targetProgress: number) => {
            if (scrollToProgressFn) {
                ...
                console.warn(
                    "scrollToProgress called but no content measured yet",
                );
            }
        },
        [scrollToProgressFn],
...
function ProgressionContent({
    ...
    const { updateProgress, setReady, setScrollToProgressFn, progressRef } =
        useProgression();
    ...
    const measureProgress = React.useCallback(() => {
        ...
        const currentPosition = Math.max(
            ...
            Math.min(
                scrollTop -
                    startPosition +
                    (progressRef.current / 100) * windowHeight,
            ),
        );
        ...
        const newProgress =
            totalHeight > 0
                ? Math.min(
                      100,
                      Math.max(0, (currentPosition / totalHeight) * 100),
                  )
                : 0;
    ...
    const scrollToProgress = React.useCallback((targetProgress: number) => {
        ...
        const targetScrollTop =
            startPosition +
            (rect.height * targetProgress) / 100 -
    ...
    return (
        <div
            className={cn(
                ...
                "bg-muted-foreground/20 overflow-hidden rounded-xl",
                className,
            )}
            style={
                direction === "horizontal"
        ...
        >
            <div
                className={cn(
                    ...
                    "bg-primary rounded-xl transition-all duration-300 ease-out",
                    barClassName,
                )}
                style={
                    direction === "horizontal"
