...
export default function View({
	...
	return (
		<AnimatePresence mode="wait">
			<motion.div key={pathname} className="relative p-2 h-[100vh] w-[100vw]">
				<motion.div {...animate(perspective)} className="bg-muted h-full rounded-2xl overflow-y-scroll w-full">
					<motion.div {...animate(opacity)}>
						<FrozenRoute>
							<div className="absolute h-[100vh] left-0 right-0 overflow-hidden rounded-2xl">
								<StaggeredMenu
									isFixed={false}
									position="right"
									items={menuItems}
				...
				<motion.div
					{...animate(slide)}
					className="fixed p-2 bg-clip-content rounded-3xl top-0 left-0 bg-muted h-full w-full"
				/>
			</motion.div>
		</AnimatePresence>
	);
